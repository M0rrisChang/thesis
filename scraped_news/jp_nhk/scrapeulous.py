...
def scrape(start_date, API_KEY_LIST):
    ...
    items = [f'site://www3.nhk.or.jp/news/html/{start_date.replace("-", "")} 新型コロナウイルス OR 新型コロナ']
        ...
        if isinstance(res['result'], list):
            for page in res['result']:
                for link in page['results']:
                    ...
                    news['url'] = url
                    news['date'] = link['date']
                    news['backup_date'] = start_date
                    news['title'] = link['title']
                    
                    try:
                        news = process_single_news(news)
                    except:
                        pass
...
def process_single_news(news):
    ...
    while i < retry_count:
        try:
            ...
            news['parsed_title'] = parsed_title
            #news['publish_date'] = article.publish_date.formatting
...
def cmd_check(date):
    ...
    while True:
        ...
        f = open(filename, 'r')
        line = f.readline()
        line = line.replace(f"'publish_date': datetime.datetime({dt.year}, {dt.month}, {dt.day}, 0, 0)", "")
        news_list = eval(line)
        if news_list == []:
