'''
Ettoday的新聞url https://www.ettoday.net/news/20200304/1659424.htm
有固定格式，理論上可以撈出所有新聞
'''
...
def scrape(start_date, end_date, API_KEY_LIST):
    url = 'https://scrapeulous.com/api'
    items = [
        f'site://www.ettoday.net 新冠肺炎 OR 武漢肺炎 OR covid OR 冠狀病毒 after:{start_date} before:{end_date}']
    ...
    path = 'tw_ettoday_news'
    filename = f'/Users/MorrisChang/thesis/scraped_news/{path}/{start_date}_{end_date}.csv'
    f = open(filename, 'w')
...
def process_news(filename, date):
    path = 'tw_ettoday_news'
    ...
    for news in filtered:
        ...
        while i < retry_count:
            try:
                ...
                error = str(e)
                if '404' in error or '503 Server Error' in error:
                    break
                import time
                time.sleep(2)
                print(f'error parsing {url} retrying .. {i}')
                i += 1
...
def cmd_scrape(date):
    while True:
        ...
        scrape(date, (dt + timedelta(days=1)).strftime('%Y-%m-%d'), API_KEY_LIST)
...
def cmd_check(date):
    ...
    while True:
        ...
        if news_list == []:
            ...
            news_num = scrape(date, end_date, API_KEY_LIST)
            if news_num == 0:
                is_all_clear = False
        else:
